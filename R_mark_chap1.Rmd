...
## Selection de variable utile (critère AIC avec D0)
...
Le tri pour la selection des variables à effet significatif par l'AIC sont: 
  Le nombre de remanent
  La proximité forestière
  Le type de sol
 
  
## Anova pour tester les variables hautement significatives
...
Finalement les variables significatives que nous integrerons dans notre modèle en plus du temps (âge de la parcelle) sont: 
...
ggplot(AIClambdaobs, aes(x=Typedesol, y=lambdaobs)) + geom_point() +  geom_boxplot()
```
...
```{r add proximity (model.v2) to D0 plot, warning=FALSE,echo=FALSE}}
...
### Type of soil (model.v3)


```{r add Type of soil (model.v3) to D0 plot, warning=FALSE,echo=FALSE}
Tresult1[Tresult1$categorie=="foretanci ",]$age<-200
Tresult1<-Tresult1[!Tresult1$categorie == "foretexpl ", ]
#Tresult1 <- rename(Tresult1, S=Typedesol)
Tresult1 <- mutate(Tresult1, S=(c(as.numeric(Tresult1$Typedesol))))

modelv3 <- stan(file = "model.v3.stan",
               data=list(N=dim(Tresult1)[1],age=Tresult1$age, D0=Tresult1$D0, S=length(unique(Tresult1$S)), sol=as.numeric(factor(Tresult1$Typedesol, levels = unique(Tresult1$Typedesol)))),
               pars=c("lambda", "alpha0", "Rsmax", "sigma", "beta_sol"),
               chains=3,
               iter=2000,
               warmup=1000)

traceplot(modelv3, pars=c("lambda", "alpha0", "Rsmax", "sigma", "beta_sol"), nrow=2)
plot(modelv3, pars=c("lambda", "alpha0", "Rsmax", "sigma", "beta_sol"))
...
parv3<-extract(modelv3)
age<-numeric()
# age <- rep(1:200, each=2)
#Typedesol <- ((rep(c(Tresult1$S:200))))
med<-numeric()
q05<-numeric()
q95<-numeric()
sol<-numeric()
...
for (i in 1:200){
  for (s in 1:2){
    age <- c(age, i)
    med<-c(med,median(
      parv3$Rsmax*(1-exp(-(parv3$lambda + (parv3$beta_sol[,s]))*i))+parv3$alpha0
    ))
    q05<-c(q05,quantile(
      parv3$Rsmax*(1-exp(-(parv3$lambda + (parv3$beta_sol[,s]))*i))+parv3$alpha0
      , probs=0.005))
...
newdatav3<- data.frame(age=age, med=med, q05=q05,
                    q95= q95, sol = as.factor(sol))
library(ggplot2)
ggplot() +
  geom_ribbon(data=newdatav3, aes(x=age, ymin = q05, ymax = q95, fill=sol), alpha = .25)+
  geom_point(data=Tresult1, aes(x = age, y=D0))+
  geom_line(data=newdatav3, aes(x=age, y=med, col=sol))+ 
  geom_boxplot(data=data5, aes(x=200, y=D0), width=0.1)+  
  scale_x_log10()+ scale_y_continuous(limits=c(0, 200))+
  labs(x="Age of Secondary Forest (yr)", y="Species Richness")
  # theme(legend.position=" none")
```


### Number of remanent and proximity (model.v12)
...
modelv12 <- stan(file = "model.v12.stan",
               data=list(N=dim(Tresult1)[1],age=Tresult1$age, D0=Tresult1$D0, remanent=Tresult1$nbr_remanent, proximity=Tresult1$proximity),
...
### Number of remanent and Type of soil (model.v13)


```{r add number of remanent and type of soil (model.v13) to D0 plot, warning=FALSE,echo=FALSE}
#Tresult1$nbr_remanent[is.na(Tresult1$nbr_remanent)] <-0
Tresult1$Proximiteforesti.re.[is.na(Tresult1$Proximiteforesti.re.)] <-0
Tresult1 <- Tresult1 %>%mutate(proximity = Proximiteforesti.re. / 1000)
Tresult1[Tresult1$categorie=="foretanci ",]$age<-200
Tresult1<-Tresult1[!Tresult1$categorie == "foretexpl ", ]
...
modelv13 <- stan(file = "model.v13.stan",
               data=list(N=dim(Tresult1)[1],age=Tresult1$age, D0=Tresult1$D0, remanent=Tresult1$nbr_remanent, S=length(unique(Tresult1$S)), sol=as.numeric(factor(Tresult1$Typedesol, levels = unique(Tresult1$Typedesol)))),
               pars=c("lambda", "alpha0", "Rsmax", "sigma", "beta0", "beta_sol"),
               chains=3,
               iter=2000,
               warmup=1000)

traceplot(modelv13, pars=c("lambda", "alpha0", "Rsmax", "sigma", "beta0", "beta_sol"), nrow=2)
plot(modelv13, pars=c("lambda", "alpha0", "Rsmax", "sigma", "beta0", "beta_sol"))
save(modelv13, file = "model.v13.Rdata")
```


```{r stan real data of D0 PLOT (modelv.13), warning=FALSE,echo=FALSE}
parv13<-extract(modelv13)
age<-1:200
#nub_remanent<-1:200
med<-numeric()
q05<-numeric()
...
med<-c(med,median(
  parv13$Rsmax*(1-exp(-(parv13$lambda + parv13$beta0 + (parv13$beta_sol[i]))*i))+parv13$alpha0
  ))
q05<-c(q05,quantile(
  parv13$Rsmax*(1-exp(-(parv13$lambda + parv13$beta0 + (parv13$beta_sol[i]))*i))+parv13$alpha0
  , probs=0.005))
...
library(ggplot2)
newdata13 <- data.frame(age=age, med=med, q05=q05,
                    q95= q95)
ggplot() +
  geom_ribbon(data=newdata13, aes(x=age, ymin = q05, ymax = q95), alpha = .25)+
  geom_point(data=Tresult1, aes(x =age, y=D0))+
  geom_line(data=newdata13, aes(x=age, y=med))+
  geom_boxplot(data=data5, aes(x=200, y=D0), width=0.1)+  
  scale_x_log10()+ scale_y_continuous(limits=c(0, 200))+
  labs(x="Age of Secondary Forest (yr)", y="Species Richness") + theme(legend.position=" none")
```
...
### Proximity and Type of soil (model.v23)
...
modelv23 <- stan(file = "model.v23.stan",
               data=list(N=dim(Tresult1)[1],age=Tresult1$age, D0=Tresult1$D0, proximity=Tresult1$proximity, S=length(unique(Tresult1$S)), sol=as.numeric(factor(Tresult1$Typedesol, levels = unique(Tresult1$Typedesol)))),
               pars=c("lambda", "alpha0", "Rsmax", "sigma", "beta0", "beta_sol"),
               chains=3,
               iter=2000,
               warmup=1000)

traceplot(modelv23, pars=c("lambda", "alpha0", "Rsmax", "sigma", "beta0", "beta_sol"), nrow=2)
plot(modelv23, pars=c("lambda", "alpha0", "Rsmax", "sigma", "beta0", "beta_sol"))
save(modelv23, file = "model.v23.Rdata")
```


```{r stan real data of D0 PLOT (modelv.23), warning=FALSE,echo=FALSE}
parv23<-extract(modelv23)
age<-1:200
#nub_remanent<-1:200
med<-numeric()
q05<-numeric()
...
med<-c(med,median(
  parv23$Rsmax*(1-exp(-(parv23$lambda + parv23$beta0 + (parv23$beta_sol[i]))*i))+parv23$alpha0
  ))
q05<-c(q05,quantile(
  parv23$Rsmax*(1-exp(-(parv23$lambda + parv23$beta0 + (parv23$beta_sol[i]))*i))+parv23$alpha0
  , probs=0.005))
...
library(ggplot2)
newdata23 <- data.frame(age=age, med=med, q05=q05,
                    q95= q95)
ggplot() +
  geom_ribbon(data=newdata23, aes(x=age, ymin = q05, ymax = q95), alpha = .25)+
  geom_point(data=Tresult1, aes(x =age, y=D0))+
  geom_line(data=newdata23, aes(x=age, y=med))+
  geom_boxplot(data=data5, aes(x=200, y=D0), width=0.1)+  
  scale_x_log10()+ scale_y_continuous(limits=c(0, 200))+
  labs(x="Age of Secondary Forest (yr)", y="Species Richness") + theme(legend.position=" none")
```
...
### Number of remanent, proximity and Type of soil (model.v123)

```{r add Number of remanent, proximity and Type of soil (model.v123) to D0  plot,warning=FALSE,echo=FALSE}
#Tresult1$nbr_remanent[is.na(Tresult1$nbr_remanent)] <-0
Tresult1$Proximiteforesti.re.[is.na(Tresult1$Proximiteforesti.re.)] <-0
Tresult1 <- Tresult1 %>%mutate(proximity = Proximiteforesti.re. / 1000)
Tresult1[Tresult1$categorie=="foretanci ",]$age<-200
...
modelv123 <- stan(file = "model.v123.stan",
               data=list(N=dim(Tresult1)[1],age=Tresult1$age, D0=Tresult1$D0,remanent=Tresult1$nbr_remanent, proximity=Tresult1$proximity, S=length(unique(Tresult1$S)), sol=as.numeric(factor(Tresult1$Typedesol, levels = unique(Tresult1$Typedesol)))),
               pars=c("lambda", "alpha0", "Rsmax", "sigma", "beta0","beta1", "beta2_sol"),
               chains=3,
               iter=2000,
               warmup=1000)

traceplot(modelv123, pars=c("lambda", "alpha0", "Rsmax", "sigma", "beta0","beta1", "beta2_sol"), nrow=2)
plot(modelv123, pars=c("lambda", "alpha0", "Rsmax", "sigma", "beta0","beta1", "beta2_sol"))
save(modelv123, file = "model.v123.Rdata")
```


```{r stan real data of D0 PLOT (modelv.123), warning=FALSE,echo=FALSE}
parv123<-extract(modelv123)
age<-1:200
#nub_remanent<-1:200
med<-numeric()
q05<-numeric()
...
med<-c(med,median(
  parv123$Rsmax*(1-exp(-(parv123$lambda + parv123$beta0 + parv123$beta1 + (parv123$beta2_sol[i]))*i))+parv123$alpha0
  ))
q05<-c(q05,quantile(
  parv123$Rsmax*(1-exp(-(parv123$lambda + parv123$beta0 + parv123$beta1 + (parv123$beta2_sol[i]))*i))+parv123$alpha0
  , probs=0.005))
...
library(ggplot2)
newdata123 <- data.frame(age=age, med=med, q05=q05,
                    q95= q95)
ggplot() +
  geom_ribbon(data=newdata123, aes(x=age, ymin = q05, ymax = q95), alpha = .25)+
  geom_point(data=Tresult1, aes(x =age, y=D0))+
  geom_line(data=newdata123, aes(x=age, y=med))+
  geom_boxplot(data=data5, aes(x=200, y=D0), width=0.1)+  
  scale_x_log10()+ scale_y_continuous(limits=c(0, 200))+
  labs(x="Age of Secondary Forest (yr)", y="Species Richness") + theme(legend.position=" none")
```
